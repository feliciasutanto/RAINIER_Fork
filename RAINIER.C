...
#include "parameters.h" // all (or most) parameters for the simulations
...
void ReadDisInputFile() {
  ifstream lvlFile;
  TString szFile = "levels/z" + TString::Format("%03d",g_nZ) + ".dat";
...
void ReadPopFile() {
  ...
  for(int binE=1; binE<g_nExPopI; binE++) {
    for(int binJ=1; binJ<=g_nSpPopIBin; binJ++) {
      ...
      // g_h2PopDist->SetBinContent(binJ,binE, 2 * adPop[binE-1][binJ-1]); // if using one party only 
    } // assign J
  } // assign E
